...
with st.form("my_profile"):
    ...
    birth_date=c3.date_input("Data de nascimento",value=parse_birth(profile.get("birth_date")),min_value=date(1940,1,1),max_value=date.today())
...
if save:
    try:
        payload={"full_name":name.strip() or profile.get("full_name") or "Sem nome","modality":modality,"stance":None if stance=="Não informado" else stance,"birth_date":birth_date.isoformat(),"city":city.strip() or None,"state":state or None}
